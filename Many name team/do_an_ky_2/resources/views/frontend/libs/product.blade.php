<div class="category">
    ...
    <div class="fr__product__inner">
        <h4 style="font-family: Arial, sans-serif;"><a href="/shop/product/{{$item->product_id}}-{{Str::slug($item->product_name, '-')}}.html">{{$item->product_name}}</a></h4>
        <ul class="fr__pro__prize">
            @if ($item->product_sale != 0)
            <li class="old__prize" style="text-decoration:line-through;">$ {{number_format($item->product_price_sell)}} </li>
            @endif
            <li>$ {{number_format($product_price_sale)}}</li>
        </ul>
    </div>
</div>
